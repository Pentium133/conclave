...
TDD-suite для **промптов сабагентов**. Все приёмы антисикофантии в `agents/*.md` (квота objections, two-pass self-rating, disagree-flag, 3-attempt → `[ASSUMED]`) — это **обещания в системных промптах**. Этот suite проверяет, что обещания срабатывают на специально подобранных адверсариальных входах.
...
- После любого редактирования `agents/*.md` (особенно `spec-skeptic`, `arch-reviewer`, `interviewer`).
