...
- **ORM / data-layer для PostgreSQL** (см. ADR-003) — отдельное архитектурное решение: TypeORM (NestJS-native, но heavyweight) / Prisma (генерация типов, отличная DX) / Drizzle (lightweight, type-safe SQL builder) / Kysely (explicit SQL) / `pg` без ORM. Выбор зависит от того, что уже используется другими микросервисами монорепо. Не блокирует ADR-001, но должен быть зафиксирован отдельным ADR (или расширением ADR-003) до старта реализации.
