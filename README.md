...
В любой сессии Claude Code (клонировать репо локально не нужно — Claude Code сам стянет его с GitHub):
...
После `install` команды `/conclave:*` (`start`, `interview`, `challenge-spec`, `architect`, `review-arch`, `implement`, `audit-code`, `status`) доступны в любом проекте — плагин живёт глобально, а артефакты пайплайна (`process/<slug>/...`) пишутся в CWD активного проекта.

**Обновление:** `/plugin marketplace update conclave` — подтянет последнюю версию из репозитория.

**Удаление:** `/plugin uninstall conclave@pentium133`.

## Что дальше читать

- Если только что поставили плагин и хотите запустить пайплайн — читайте `CLAUDE.md`.
- Этот README отвечает на вопрос «как устроен процесс и почему именно так».
...
#### Регрессионная проверка антисикофантии

Все приёмы выше — это **обещания в промптах**. Чтобы убедиться, что они срабатывают на адверсариальных входах (а не только звучат красиво), в `tests/prompts/` лежит TDD-suite из трёх фикстур:

1. **Evasive developer** — заготовленные уклончивые ответы. Проверяет, что `interviewer` доходит до `[ASSUMED]` после 3 попыток, а не молча принимает «TBD».
2. **Soft spec** — нарочито мягкая спецификация с дырами. Проверяет, что `spec-skeptic` выдаёт ≥7 objections, ≥5 deep/medium после Pass 2, и НЕ возвращает `approve-with-notes`.
3. **Plausible ADRs** — ADR-ы с скрытыми операционными ловушками (нет джиттера, нет circuit-breaker, нет heartbeat у SSE). Проверяет, что `arch-reviewer` заполняет per-ADR `disagree-flag` непустым значением для каждого ADR.

Запуск: `bash tests/prompts/run-all.sh` — однокомандный (headless), прогоняет 02-soft-spec и 03-plausible-adrs автоматически через `claude -p`. Фикстура 01-evasive-developer требует интерактивного диалога — для неё передайте `--interactive` и держите `claude`-сессию открытой. Suite проверяет инварианты артефактов через `grep` — не точный текст, а **счётчики и наличие обязательных секций**. Это и есть критерий «промпт держит роль»: если живая модель на адверсариальном входе не выдаёт минимально требуемого, антисикофантия сломалась.

CI: `.github/workflows/prompt-regression.yml`, триггер `workflow_dispatch`, требует секрет `ANTHROPIC_API_KEY`. Не запускается на каждый PR (стоит токенов).
