...
Запуск: `bash tests/prompts/run-all.sh`. Suite интерактивный (требует открытой `claude` сессии для исполнения команд) и проверяет инварианты артефактов через `grep` — не точный текст, а **счётчики и наличие обязательных секций**. Это и есть критерий «промпт держит роль»: если живая модель на адверсариальном входе не выдаёт минимально требуемого, антисикофантия сломалась.

CI: `.github/workflows/prompt-regression.yml` с триггером `workflow_dispatch` (вручную из GitHub UI). Не запускается на каждый PR — каждый прогон стоит токенов, и для тестового задания достаточно ручного smoke перед сдачей.
